...
def apply(img, gt_boxes):
    ...
    inputs:
        ...
        gt_boxes = (ground_truth_object_count, [x1, x2, y1, y2])
            in normalized form [0, 1]
    outputs:
        modified_img = (final_height, final_width, depth)
        modified_gt_boxes = (ground_truth_object_count, [x1, x2, y1, y2])
            in normalized form [0, 1]
    """
    # Color operations
...
def randomly_apply_operation(operation, img, gt_boxes, *args):
    ...
    outputs:
        ...
        modified_or_not_gt_boxes = (ground_truth_object_count, [x1, x2, y1, y2])
    """
    return tf.cond(
        get_random_bool(),
...
def flip_horizontally(img, gt_boxes):
    ...
    flipped_gt_boxes = tf.stack([1.0 - gt_boxes[..., 0],
                                 gt_boxes[..., 1],
                                 1.0 - gt_boxes[..., 2],
                                 gt_boxes[..., 3]], -1)
