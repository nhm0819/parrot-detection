...
def generate_iou_map(bboxes, gt_boxes, transpose_perm=[0, 2, 1]):
    ...
    # x_top = tf.maximum(bbox_x1, gt_x1)
    # y_top = tf.maximum(bbox_y1, gt_y1)
    # x_bottom = tf.minimum(bbox_x2, gt_x2)
    # y_bottom = tf.minimum(bbox_y2, gt_y2)
...
def get_bboxes_from_deltas(prior_boxes, deltas):
    ...
    #
    all_bbox_width = tf.exp(deltas[..., 2]) * all_pbox_width
    all_bbox_height = tf.exp(deltas[..., 3]) * all_pbox_height
    all_bbox_ctr_x = (deltas[..., 0] * all_pbox_width) + all_pbox_ctr_x
    all_bbox_ctr_y = (deltas[..., 1] * all_pbox_height) + all_pbox_ctr_y
    #
